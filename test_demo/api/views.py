...
from rest_framework.status import HTTP_200_OK, HTTP_201_CREATED, HTTP_403_FORBIDDEN, HTTP_404_NOT_FOUND,HTTP_401_UNAUTHORIZED, HTTP_422_UNPROCESSABLE_ENTITY
...
def changeProduct(request, pk):
    ...
    if __name__ == '__main__':
        if request.method == "DELETE":
            product.delete()
            return Response({'data': {'massage': 'Product removed'}}, status=HTTP_200_OK)
            ...
            return Response({'data': {'code': 422, 'message': 'Validation error', 'error': prod_ser.errors}}, status=HTTP_422_UNPROCESSABLE_ENTITY)
...
def createProduct(request):
    ...
    if prod_ser.is_valid():
        return Response({'data': {'id': prod_ser.data['id'], 'message': 'Product added'}}, status=HTTP_201_CREATED)
    else:
        return Response({'error': {'code': 422, 'message': 'Validation error', 'error': prod_ser.errors}}, status=HTTP_422_UNPROCESSABLE_ENTITY)
